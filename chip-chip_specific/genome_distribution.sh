...
bedtools intersect -wao -a $INFILE -b $GENOMELOCS | cut -f 1,2,3,9 > ${OUTDIR}/${BASENAME}.location.bed

echo "overlaping $BASENAME_SHUFFLED with genomic locations"	
bedtools intersect -wao -a ${OUTDIR}/$BASENAME_SHUFFLED.bed -b $GENOMELOCS | cut -f 1,2,3,9 > ${OUTDIR}/${BASENAME_SHUFFLED}.location.bed
