...
awk '{print $1, int(($2+$3)/2), int((($2+$3)/2)+1), $4, $5}' $INFILE | sed '{:q;N;s/ /\t/g;t q}' > ${OUTDIR}/${BASENAME}.summits.bed

# generate shuffled peaks
echo "generating random peaks for $BASENAME"
bedtools shuffle -i ${OUTDIR}/${BASENAME}.summits.bed -g $GENOME -excl $EXCLUDE -seed 0 | bedtools sort > ${OUTDIR}/$BASENAME_SHUFFLED.bed

# Intersect 
# reports an entry for all features (even not intercepting, 0)
echo "overlaping $BASENAME with genomic locations"	
bedtools intersect -wao -a ${OUTDIR}/${BASENAME}.summits.bed -b $GENOMELOCS | cut -f 1,2,3,5,9 > ${OUTDIR}/${BASENAME}.location.bed
...
bedtools closest -d -a ${OUTDIR}/${BASENAME}.summits.bed -b $TSSs | cut -f 1,2,7,9,11 > ${OUTDIR}/${BASENAME}.TSS.bed
