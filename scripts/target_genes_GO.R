...
gene_IDs = unique(CherGenes$gene)
...
GO_BP<-GO.analysis(as.list(gene_IDs), ontol="BP", direction="over", universe=as.list(universe))
GO_BP<-GO.analysis(universe[1:1000], ontol="BP", direction="over", universe=as.list(universe))

## Tests for over-represented biological process, molecular function and cellular component terms:
GO_BP<-GO.analysis(genes=as.list(gene_IDs), ontol="BP", direction="over", universe=universe) #testing for overrepresentation here, but underrep also possible (or both)
GO_MF<-GO.analysis(genes=as.list(gene_IDs), ontol="MF", direction="over", universe=universe)
GO_CC<-GO.analysis(genes=as.list(gene_IDs), ontol="CC", direction="over", universe=universe)
